...
def init_db():
    ...
    if not cur.fetchone():
        cur.execute("INSERT INTO admins (name,email,password_hash) VALUES (%s,%s,%s)",
                    ('Admin', 'admin@canteen.com', generate_password_hash('admin123')))

    # Default settings
    for k, v in [('canteen_name','College Canteen'),('college_name','Engineering College'),
    ...
    if cur.fetchone()['c'] == 0:
        items = [
            ('Idli Sambar','Breakfast',30.00,'Soft idlis with sambar',50,'08:00','11:00'),
            ...
            ('Samosa','Snacks',15.00,'Crispy fried pastry',60,'10:00','18:00'),
        ...
        for item in items:
            cur.execute("""INSERT INTO menu_items
                (name,category,price,description,available_quantity,available_from,available_to)
    ...
    if cur.fetchone()['c'] == 0:
        ...
        for s in students:
            cur.execute("""INSERT INTO students
                (name,roll_number,branch,year,phone,email,prepaid_balance)
                VALUES (%s,%s,%s,%s,%s,%s,%s)""", s)

    conn.commit()
    cur.close()
    conn.close()
    app.logger.info("Database initialised successfully.")
...
def internal_error(e):
    ...
    return jsonify({'error': '500', 'detail': str(e)}), 500
